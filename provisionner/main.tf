...
#empty container the provivioner itself should be under the ressources
#but her we will put it under empty container called null resources
resource "null_resource" "n1" {  
  ...
  # if we put local-exec any provisioner will run on a local machine
  provisioner "local-exec" {
    command = "echo hello" 
  }
  # if we put remote-exec any provisioner will run on a local machine
